...
#define SELECT()	PORTCbits.RC6 = 0		/* CS = L */
#define	DESELECT()	PORTCbits.RC6 = 1		/* CS = H */
#define	MMC_SEL		!PORTCbits.RC6	/* CS status (true:CS == L) */
...
/* PANCEAC send acmd to avoid recursion*/
static
BYTE send_acmd (
...
DSTATUS disk_initialize (void)
{
    ...
    send_cmd(CMD0, 0); send_cmd(CMD0, 0);
	ty = 0;
	if (send_cmd(CMD0, 0) == 1) {			/* Enter Idle state */
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2 */
			for (n = 0; n < 4; n++) ocr[n] = rcv_spi();		/* Get trailing return value of R7 resp */
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {			/* The card can work at vdd range of 2.7-3.6V */
				for (tmr = 10000; tmr && send_cmd(ACMD41, 1UL << 30); tmr--) dly_us(100);	/* Wait for leaving idle state (ACMD41 with HCS bit) */
			...
			for (tmr = 10000; tmr && send_cmd(cmd, 0); tmr--) dly_us(100);	/* Wait for leaving idle state */
	...
	if (buff) {		/* Send data bytes */
		...
		if (sc) {	/* Initiate sector write process */
			...
			if ((rcv_spi() & 0x1F) == 0x05) {	/* Receive data resp and wait for end of write process in timeout of 500ms */
				for (bc = 5000; rcv_spi() != 0xFF && bc; bc--) dly_us(100);	/* Wait ready */
				if (bc) res = RES_OK;
			}
			DESELECT();
			rcv_spi();
